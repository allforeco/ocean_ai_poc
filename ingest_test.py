#!/usr/bin/env python3
"""
Test version of ingest.py with dummy embeddings for demonstration
"""
...
class DocumentIngestorTest:
    ...
    def create_dummy_embeddings(self, texts: List[str]) -> List[List[float]]:
        """Create dummy embeddings for testing (1536 dimensions like OpenAI)"""
        embeddings = []
        for text in texts:
            # Create a deterministic but random-looking embedding based on text hash
            text_hash = hashlib.md5(text.encode()).hexdigest()
            random.seed(int(text_hash[:8], 16))
            embedding = [random.uniform(-1, 1) for _ in range(1536)]
            embeddings.append(embedding)
        print(f"Created dummy embeddings for {len(texts)} texts")
        return embeddings
    ...
    def ingest_document(self, file_path: str, organization: str = None) -> bool:
        ...
        all_embeddings = self.create_dummy_embeddings(chunks)
...
def main():
    parser = argparse.ArgumentParser(description="Test document ingestion with dummy embeddings")
    ...
    ingestor = DocumentIngestorTest(args.config)
